...
from database_setting import get_bitcoin_data, store_prediction
...
class Conversation:
    ...
    def calculate_daily_return(self, btc_open, btc_close):
    ...
    def send(self, analysis_date=None):
        ...
        daily_return = self.calculate_daily_return(btc_open, btc_close)

        # Criação de um DataFrame para exibir os dados, incluindo a rentabilidade
        df = pd.DataFrame({
            'Data': [actual_date],
            'Recomendação': [recommendation.group(1) if recommendation else 'N/A'],
            'Nível de Confiança': [trust_rate.group(1) if trust_rate else 'N/A'],
            'Valor do Bitcoin': [value_btc.group(1) if value_btc else 'N/A'],
            'Stop Loss': [stop_loss.group(1) if stop_loss else 'N/A'],
            'Take Profit': [take_profit.group(1) if take_profit else 'N/A'],
            'Relação Risco/Recompensa': [risk_return.group(1) if risk_return else 'N/A'],
            'Rentabilidade Diária (%)': [daily_return]
        })

        # Exibindo o DataFrame no UI (Streamlit ou outro sistema de exibição)
        try:
            import ace_tools as tools  # Supondo que ace_tools seja uma ferramenta para exibição no UI
            tools.display_dataframe_to_user(name="Análise de Swing Trading", dataframe=df)
        except Exception as e:
            print(f"Erro ao exibir dados no UI: {e}")
